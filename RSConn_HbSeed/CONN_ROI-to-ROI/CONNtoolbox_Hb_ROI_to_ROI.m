...
TARGETpath='/nafs/narr/HCP_OUTPUT/Habenula/outputs/RSConn_HbSeed/CONN_ROI_to_ROI';                                                             % target folder for conn project (default current folder)
...
NSUBJECTS=1;                                                               % number of subjects to include in your project (leave empty for all subjects)
...
for n=1:numel(subs)
    ...
    f1=fullfile(sprintf(CONNECTOMEpath,subs{n}),'Results','rest_acq-AP_run-01','rest_acq-AP_run-01_hp2000_clean.nii.gz');   % FUNCTIONAL VOLUME (1/4)
    f2=fullfile(sprintf(CONNECTOMEpath,subs{n}),'Results','rest_acq-PA_run-02','rest_acq-PA_run-02_hp2000_clean.nii.gz');   % FUNCTIONAL VOLUME (2/4)
    r1=fullfile(sprintf(CONNECTOMEpath,subs{n}),'Results','rest_acq-AP_run-01','Movement_Regressors.txt');              % REALIGNMENT FILE (1/4)
    r2=fullfile(sprintf(CONNECTOMEpath,subs{n}),'Results','rest_acq-AP_run-01','Movement_Regressors.txt');              % REALIGNMENT FILE (2/4)
...
batch.Setup.outputfiles=[1,1,0,0,0,1];
...
batch.Setup.preprocessing.steps={'functional_smooth'};  % Run additional preprocessing steps: segmentation,ART,smoothing
batch.Setup.preprocessing.fwhm=5;                       % smoothing fwhm (mm)
conn_batch(batch);
...
batch.Analysis.measure=[1,2];
batch.Analysis.type=1;
%batch.Analysis.sources= seed rois if empty will compute for all rois
clear batch;
