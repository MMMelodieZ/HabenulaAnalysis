function Denoising_CONNtoolbox_HbRSConn_SHELL(SUBJID)
...
TARGETpath='/nafs/narr/HCP_OUTPUT/Habenula/outputs/RSConn_HbSeed/CONNToolboxOutputs';                                                             % target folder for conn project (default current folder)
...
%subs=dir(regexprep(CONNECTOMEpath,'%s.*$','k*')); 
subs=dir(regexprep(CONNECTOMEpath,'%s.*$',SUBJID));
subs=dir(regexprep(subsdir,'%s.*$','*')); 
subs=subs([subs.isdir]>0);
subs={subs.name};
subs=subs(cellfun(@(s)all(s>='5'&s<='7'),subs));


%subs=subs([subs.isdir]>0);
%SUBJID='k000401'
%subs=SUBJID;
%subs=subs(cellfun(@(s)all(s>='0'&s<='7'),subs));
%if isempty(NSUBJECTS), NSUBJECTS=1; 
...
%for n=1:numel(subs)
    ...
    t1=fullfile(sprintf(CONNECTOMEpath,SUBJID),'T1w_restore_brain.nii.gz');                                        % STRUCTURAL VOLUME
    f1=fullfile(sprintf(CONNECTOMEpath,SUBJID),'Results','rest_acq-AP_run-01','rest_acq-AP_run-01_hp2000_clean.nii.gz');   % FUNCTIONAL VOLUME (1/4)
    f2=fullfile(sprintf(CONNECTOMEpath,SUBJID),'Results','rest_acq-PA_run-02','rest_acq-PA_run-02_hp2000_clean.nii.gz');   % FUNCTIONAL VOLUME (2/4)
    r1=fullfile(sprintf(CONNECTOMEpath,SUBJID),'Results','rest_acq-AP_run-01','Movement_Regressors.txt');              % REALIGNMENT FILE (1/4)
    r2=fullfile(sprintf(CONNECTOMEpath,SUBJID),'Results','rest_acq-AP_run-01','Movement_Regressors.txt');              % REALIGNMENT FILE (2/4)
    ...
    if COPYFILES
        ...
        [ok,nill]=mkdir(fullfile(TARGETpath,'LocalCopyDataFiles'),SUBJID);
        t1b=fullfile(TARGETpath,'LocalCopyDataFiles',SUBJID,'structural.nii.gz');  if OVERWRITE||isempty(dir(t1b)), [ok,nill]=system(sprintf('cp ''%s'' ''%s''',t1,t1b)); end; t1=t1b;
        f1b=fullfile(TARGETpath,'LocalCopyDataFiles',SUBJID,'functional1.nii.gz'); if OVERWRITE||isempty(dir(f1b)), [ok,nill]=system(sprintf('cp ''%s'' ''%s''',f1,f1b)); end; f1=f1b;
        f2b=fullfile(TARGETpath,'LocalCopyDataFiles',SUBJID,'functional2.nii.gz'); if OVERWRITE||isempty(dir(f2b)), [ok,nill]=system(sprintf('cp ''%s'' ''%s''',f2,f2b)); end; f2=f2b;
        r1b=fullfile(TARGETpath,'LocalCopyDataFiles',SUBJID,'Movement1.txt'); if OVERWRITE||isempty(dir(r1b)), [ok,nill]=system(sprintf('cp ''%s'' ''%s''',r1,r1b)); end; r1=r1b;
        r2b=fullfile(TARGETpath,'LocalCopyDataFiles',SUBJID,'Movement2.txt'); if OVERWRITE||isempty(dir(r2b)), [ok,nill]=system(sprintf('cp ''%s'' ''%s''',r2,r2b)); end; r2=r2b;
    end
    ...
    FUNCTIONAL_FILE(1,1:2)={f1,f2};
    REALIGNMENT_FILE(1,1:2)={r1,r2};
%end
nsessions=2;
