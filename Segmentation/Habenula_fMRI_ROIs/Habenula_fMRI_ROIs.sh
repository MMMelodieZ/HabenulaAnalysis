...
for hemi in left right ; do
	...
	echo $(echo ${max}-${min} | bc)
	echo $workdir/${sub}_${hemi}_Hb_region_func_unscaled.nii.gz
	fslmaths $workdir/${sub}_${hemi}_Hb_region_func_unscaled.nii.gz -sub $min -div $(echo ${max}-${min} | bc) -thr 0 \
...
fslmaths $odir/${sub}_bilat_Hb_region_full_prob -thr 0.25 -bin $odir/${sub}_bilat_shape_optimized_Hb_ROI_ALLTPsAveraged
